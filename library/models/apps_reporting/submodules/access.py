...
class ReportingApplicationPages(models.Model):
    ...
    class Meta:
        app_label   = 'apps_reporting'
        db_table    = 'pages_app'
        ordering    = ['page_order','app_code', 'page_code']
        verbose_name        = 'Reporting - Application page'
        verbose_name_plural = 'Reporting - Application pages'
    def __str__(self):
        return '(%s) %s -- %s'%(self.app_code, self.page_code, self.name)

class ReportingApplicationPageConfig(models.Model):
    access_name     = models.CharField(max_length=200)
    page            = models.ForeignKey(ReportingApplicationPages, on_delete=models.CASCADE)
    description     = models.TextField(blank=True, null=True)
    ...
    class Meta:
        ...
        verbose_name        = 'Reporting - Application page config'
        verbose_name_plural = 'Reporting - Application pages config'
    def __str__(self):
        return '(%s) %s -- %s'%(self.page, self.access_name, self.description)

class ReportingAccessRightConfig(models.Model):
    ...
    class Meta:
        ...
        verbose_name        = 'Reporting - Access right'
        verbose_name_plural = 'Reporting - Access right'
    def __str__(self):
