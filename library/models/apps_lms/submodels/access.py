...
class LMSApplicationPages(models.Model):
    ...
    class Meta:
        app_label   = 'apps_lms'
        db_table    = 'lms_pages_app'
        ordering    = ['page_order','app_code', 'page_code']
        verbose_name        = 'LMS - Application page'
        verbose_name_plural = 'LMS - Application pages'
    def __str__(self):
        return '(%s) %s -- %s'%(self.app_code, self.page_code, self.name)

class LMSAccessRightConfig(models.Model):
    access_name     = models.CharField(max_length=50)
    page            = models.ForeignKey(LMSApplicationPages, on_delete=models.CASCADE)
    access_right    = models.CharField(max_length=500)

    class Meta:
        app_label   = 'apps_lms'
        db_table    = 'lms_access_right'
        ordering    = ['page', 'access_name','access_right']
        verbose_name        = 'LMS - Access right'
        verbose_name_plural = 'LMS - Access right'
    def __str__(self):
...
class V_ApplicationPages(models.Model):
    ...
    class Meta:
        app_label   = 'apps_lms'
        db_table    = 'v_lms_application_list'
        managed     = False