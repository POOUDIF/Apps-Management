...
class HrisApplicationPages(models.Model):
    ...
    class Meta:
        app_label   = 'bbicore'
        db_table    = 'hris_pages_app'
        ordering    = ['page_order','app_code', 'page_code']
        verbose_name        = 'Hris - Application page'
        verbose_name_plural = 'Hris - Application pages'
    def __str__(self):
        return '(%s) %s -- %s'%(self.app_code, self.page_code, self.name)

class HrisApplicationPageConfig(models.Model):
    access_name     = models.CharField(max_length=200)
    page            = models.ForeignKey(HrisApplicationPages, on_delete=models.CASCADE)
    description     = models.TextField(blank=True, null=True)

    class Meta:
        app_label   = 'bbicore'
        db_table    = 'hris_page_config'
        ordering    = ['page', 'access_name']
        verbose_name        = 'Hris - Application page config'
        verbose_name_plural = 'Hris - Application pages config'
    def __str__(self):
        return '(%s) %s -- %s'%(self.page, self.access_name, self.description)

class HrisAccessRightConfig(models.Model):
    ...
    class Meta:
        app_label   = 'bbicore'
        db_table    = 'hris_access_right'
        ordering    = ['page', 'access_name','access_right']
        verbose_name        = 'Hris - Access right'
        verbose_name_plural = 'Hris - Access right'
    def __str__(self):
        return '(%s) %s -- %s'%(self.page, self.access_name, self.access_right)

class V_HrisAccessRight(models.Model):
    ...
    class Meta:
        app_label   = 'bbicore'
        db_table    = 'v_hris_access_right'
        managed     = False

class V_HrisApplicationPages(models.Model):
    access_name     = models.CharField(max_length=50)
    app_code        = models.CharField(max_length=50)
    page_code       = models.CharField(max_length=50)
    name            = models.CharField(max_length=200)
    ...
    class Meta:
        app_label   = 'bbicore'
        db_table    = 'v_hris_application_list'
        managed     = False